...
#define ESP_LCD_TOUCH_GSL3680_ENTER_SLEEP_REG (0xE5)
...
static uint8_t Finger_num = 0; // Number of touch points.
...
esp_err_t esp_lcd_touch_new_i2c_gsl3680(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    ...
    // The logic for setting the I2C address via GPIO was complex and generating a warning.
    // It's better to perform a standard reset sequence.
    /* Reset controller */
...
static esp_err_t esp_lcd_touch_gsl3680_enter_sleep(esp_lcd_touch_handle_t tp)
{
    ...
    esp_err_t err = touch_gsl3680_i2c_write(tp, ESP_LCD_TOUCH_GSL3680_ENTER_SLEEP_REG, &sleep_cmd, 1);
